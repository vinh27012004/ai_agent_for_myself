...
Bạn có các công cụ: quản lý công việc/nhắc nhở, trí nhớ dài hạn, đọc file, và sắp xếp file/thư mục.
Nguyên tắc:
...
def main() -> None:
    ...
    while True:
        ...
        if user in ("/quit", "/exit"):
            ...
            break
        if user == "/help":
            print("Lệnh: /reset (xoá hội thoại), /tools (liệt kê tool), /quit (thoát)")
            continue
        if user == "/reset":
            messages = [{"role": "system", "content": SYSTEM_PROMPT}]
